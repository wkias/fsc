...
void OLED_PrintImage(uint8 *pucTable, uint16 usRowNum, uint16 usColumnNum)
{
  ...
  m = usRowNum >> 3; //计算图片行数以8位为一组完整的组数 m=7
  n = usRowNum % 8;  //计算分完组后剩下的行数 n=4
...
void Eighty_Sixty_Display(uint8 *img)
{
  ...
  for (i = 0; i <= 6; i++) //7次循环完成56*80的图像区
  {
    ...
    {
      uint8 x = tempimg[i * 8][j] + tempimg[i * 8 + 1][j] * 2 + tempimg[i * 8 + 2][j] * 4 + tempimg[i * 8 + 3][j] * 8 + tempimg[i * 8 + 4][j] * 16 + tempimg[i * 8 + 5][j] * 32 + tempimg[i * 8 + 6][j] * 64 + tempimg[i * 8 + 7][j] * 128; //将竖着的8个像素合并成一个字节
      LCD_WrDat(x);
    }
  }
  ...
  for (j = 0; j < 80; j++)
  {
    uint8 x = tempimg[56][j] + tempimg[57][j] * 2 + tempimg[58][j] * 4 + tempimg[59][j] * 8;
    LCD_WrDat(x);
  }
}
